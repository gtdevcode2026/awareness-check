...
App.NewsletterBuilder = (() => {
  ...
  const TEMPLATE_CATALOG = [
    ...
    { id: 'gen_cybershield', name: 'Phishing Maestro', tags: ['awareness', 'bulletin', 'generated'], channels: ['email-safe', 'screen-safe'], visualProfile: 'generated', recommended: 'Dark/gold security awareness bulletin with threat-alert hero, red-flag list, and two article cards.' },
  ...
  // A "poster" is a single-subject display template (the poster slider on the
  // home picker). Identified by the catalog tags: 'poster-first' (the impact /
  // awareness-wall posters) or 'poster' (the generated gen_* posters). Excludes
  // Corporate Alert and the gen_chase_email / gen_cybershield bulletins, which
  // are multi-article newsletters. Used to cap article selection at one.
  function isPosterTemplate(id) {
    const entry = TEMPLATE_CATALOG.find((t) => t.id === id);
    if (!entry) return false;
    const tags = Array.isArray(entry.tags) ? entry.tags : [];
    return tags.includes('poster-first') || tags.includes('poster');
  }
...
@media(prefers-reduced-motion:reduce){*{animation:none!important;transition:none!important}}
  ...
  //   js/newsletter/bank_page.js      → phishingbrief, bankpage1_dynamic
